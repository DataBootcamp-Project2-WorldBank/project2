...
if (document.getElementById("gdp-chart")) {
console.log("gdp-chart in DOM");
var svg = d3.select("#gdp-chart")
  .append("svg")
  .attr("width", svgWidth)
...
// Import Data
  d3.json(stats_url).then( function(statsData) {
    ...
    var xLogScale = d3.scaleLinear()
      .domain([d3.min(statsData, d => d.gdp)*0.8, d3.max(statsData, d => d.gdp)*1.2])
      .range([0, width]);
    ...
    var toolTip = d3.tip()
      ...
      .offset([80, -60])
      .html(function(d) {
        return (`${d.country_name}<br>Gross Domestic Product: ${Math.round(d.gdp)}<br>% Satisfactory: ${d.total > 0 ? Math.round((d.satisfactory/d.total) * 100) : 0}`);
      });
